...
public class WatchBoard extends View {
    ...
    private void obtainStyledAttrs(AttributeSet attrs) {
        ...
        try {
            ...
            mPadding = array.getDimension(R.styleable.WatchBoard_wb_padding, PublicUtils.dp2px(getContext(), 10));
            mTextSize = array.getDimension(R.styleable.WatchBoard_wb_text_size, PublicUtils.dp2px(getContext(), 16));
            mHourPointWidth = array.getDimension(R.styleable.WatchBoard_wb_hour_pointer_width, PublicUtils.dp2px(getContext(), 5));
            mMinutePointWidth = array.getDimension(R.styleable.WatchBoard_wb_minute_pointer_width, PublicUtils.dp2px(getContext(), 3));
            mSecondPointWidth = array.getDimension(R.styleable.WatchBoard_wb_second_pointer_width, PublicUtils.dp2px(getContext(), 2));
            mPointRadius = (int) array.getDimension(R.styleable.WatchBoard_wb_pointer_corner_radius, PublicUtils.dp2px(getContext(), 10));
            mPointEndLength = array.getDimension(R.styleable.WatchBoard_wb_pointer_end_length, PublicUtils.dp2px(getContext(), 10));
            ...
            //一旦出现错误全部使用默认值
            mPadding = PublicUtils.dp2px(getContext(), 10);
            mTextSize = PublicUtils.dp2px(getContext(), 16);
            mHourPointWidth = PublicUtils.dp2px(getContext(), 5);
            mMinutePointWidth = PublicUtils.dp2px(getContext(), 3);
            mSecondPointWidth = PublicUtils.dp2px(getContext(), 2);
            mPointRadius = (int) PublicUtils.dp2px(getContext(), 10);
            mPointEndLength = PublicUtils.dp2px(getContext(), 10);
...
//        postInvalidateDelayed(1000);
    ...
    private void paintScale(Canvas canvas) {
        mPaint.setStrokeWidth(PublicUtils.dp2px(getContext(), 1));
        int lineWidth = 0;
        for (int i = 0; i < 60; i++) {
            if (i % 5 == 0) { //整点
                mPaint.setStrokeWidth(PublicUtils.dp2px(getContext(), 1.5f));
                ...
                canvas.save();
                canvas.translate(0, -mRadius + PublicUtils.dp2px(getContext(), 5) + lineWidth + mPadding + (textBound.bottom - textBound.top) / 2);
                mPaint.setStyle(Paint.Style.FILL);
                canvas.rotate(-6 * i);
